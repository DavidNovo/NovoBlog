		...
		<div id="list-author" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="url" title="${message(code: 'author.url.label', default: 'Url')}" />
					
						<g:sortableColumn property="email" title="${message(code: 'author.email.label', default: 'Email')}" />
					
						<th><g:message code="author.comment.label" default="Comment" /></th>
				...
				<g:each in="${authorInstanceList}" status="i" var="authorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: authorInstance, field: "url")}</td>
					
						<td>${fieldValue(bean: authorInstance, field: "email")}</td>
					
						<td>${fieldValue(bean: authorInstance, field: "comment")}</td>
